...
export default function IdCardPage({ loggedInUser, onClose }) {
  const [allEmployees, setAllEmployees] = useState(() => loadAttendanceData(loggedInUser).employees);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeAttendanceData(loggedInUser, (remoteData) => {
      if (active) { setAllEmployees(remoteData.employees); setEmployees(activeEmployeesOnly(remoteData.employees)); }
    });
  ...
  return <section className="attendance-page id-card-page">
    ...
    <header className="id-card-page-header"><div><p className="attendance-kicker">Employee identity</p><h1>ID Cards</h1><p>Print matching front and reverse sides for every employee.</p></div><div className="id-card-actions">{employees.length > 0 && <button type="button" className="attendance-primary" onClick={printIdCards}>Print / Save PDF</button>}</div></header>
    {verifiedEmployee && <section className={`id-card-verification-result ${verifiedEmployeeActive ? '' : 'id-card-verification-result--inactive'}`}><span>EMPLOYEE VERIFICATION</span><h2>{verifiedEmployee.name}</h2><p><b>ID:</b> {automaticEmployeeId(verifiedEmployee, dealerCode)}</p><p><b>Position:</b> {verifiedEmployee.designation || 'Employee'}</p><strong>{verifiedEmployeeActive ? '✓ Active employee record' : '⚠ Inactive employee record — ID card is not valid for current use'}</strong></section>}
