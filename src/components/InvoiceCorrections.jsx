...
export default function InvoiceCorrections({ view, records, disabled, mutate, onSaved }) {
  ...
  if (view === 'Outstanding Ageing') {
    ...
    return <section className="invoice-workspace__card"><h3>Outstanding Ageing</h3><p>Ageing uses the due date, or invoice date when no due date is set.</p><div className="invoice-workspace__form-grid">{['Not due', '0–30 days', '31–60 days', '61–90 days', '90+ days'].map((bucket) => <div key={bucket}><h4>{bucket}</h4><strong>{money(rows.filter((row) => row.bucket === bucket).reduce((sum, row) => sum + row.due, 0))}</strong></div>)}</div><div className="invoice-workspace__table-scroll"><table className="data-table"><thead><tr><th>Invoice</th><th>Consumer</th><th>Days overdue</th><th>Ageing</th><th>Outstanding</th></tr></thead><tbody>{rows.map(({ record, days, bucket, due }) => <tr key={record.id}><td>{record.invoiceNumber}</td><td>{record.header?.name}</td><td>{days}</td><td>{bucket}</td><td>{money(due)}</td></tr>)}</tbody></table></div></section>;
  }
