...
const blankDraft = { name: '', employeeCode: '', contact: '', designation: '', vehicleNumber: '', address: '', wageType: 'Monthly Wage', wageAmount: '', photoDataUrl: '', dob: '', gender: '', bloodGroup: '', emergencyContact: '', currentAddress: '', permanentAddress: '', monthlyBonus: '', advanceInstallment: '', manualDeduction: '', bankAccount: '', ifsc: '', upi: '', pfNumber: '', esiNumber: '' };
const DESIGNATIONS = ['Manager', 'Computer Operator', 'Mobile Operator', 'Deliveryman', 'Mechanic', 'Go-down Keeper', 'Office Staff'];
  ...
  return <section className={`attendance-page employee-profile-page ${profileEditing ? 'is-editing' : ''}`}>
    ...
    <section className="employee-profile-workspace">
      ...
      {!employee ? (createNew ? <>
        ...
        {String(draft.designation || '').toLowerCase().replace(/[-\s]/g, '') === 'deliveryman' && <section className="profile-section--full"><span className="section-label">DELIVERY VEHICLE</span><h2>Vehicle assignment</h2><div className="profile-fields profile-fields--three"><label>Vehicle number<input value={draft.vehicleNumber} onChange={(event) => setDraft({ ...draft, vehicleNumber: event.target.value.toUpperCase() })} placeholder="Vehicle number" /></label></div></section>}
        ...
        {String(draft.designation || '').toLowerCase().replace(/[-\s]/g, '') === 'deliveryman' && <section className="profile-section--full"><span className="section-label">DELIVERY VEHICLE</span><h2>Vehicle assignment</h2><div className="profile-fields profile-fields--three"><label>Vehicle number<input value={draft.vehicleNumber} onChange={(event) => setDraft({ ...draft, vehicleNumber: event.target.value.toUpperCase() })} readOnly={!profileEditing} placeholder="Vehicle number" /></label></div></section>}
